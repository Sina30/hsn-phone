...
HSN.GetPlayerData = function(identifier)
    if PhoneData[identifier] == nil then
        ...
        exports.oxmysql:insert('UPDATE users SET phonedata = @phonedata WHERE identifier = @identifier', {
...
HSN.GenerateIban = function(identifier)
    ...
    exports.oxmysql:insert('UPDATE users SET iban = @iban WHERE identifier = @identifier', {
        ['@iban'] = iban,
        ['@identifier'] = identifier
    }) 
    return iban
...
HSN.GeneratePhoneNumber = function(identifier) 
    ...
    exports.oxmysql:insert('UPDATE users SET phone_number = @phone_number WHERE identifier = @identifier', {
...
AddEventHandler("hsn-phone-server-changePhoto",function(index, photo)
    ...
    if index ~= nil then
        ...
        elseif index == "twitter" then
            ...
            exports.oxmysql:insert('UPDATE hsn_phone_twitter_accounts SET photo = @photo WHERE username = @username', {
                ['@photo'] = photo,
                ['@username'] = Username
            })
...
AddEventHandler("hsn-phone-server-deleteApp",function(app)
    ...
    for k,v in pairs(PhoneData[Ply.identifier].Apps) do
        if v == app then
            ...
            exports.oxmysql:insert('UPDATE users SET downloadedapps = @downloadedapps WHERE identifier = @identifier', {
...
ESX.RegisterServerCallback("hsn-phone-server-TwitterAccountCheck",function(source,cb,data)
    ...
    if TwitterAccounts[data.username] == nil then
        ...
        if TwitterAccounts[data.username].password == data.password then
            ...
            exports.oxmysql:insert('UPDATE hsn_phone_twitter_accounts SET owner = @owner WHERE username = @username', {
...
AddEventHandler("hsn-phone-server-registerTwitter-confirm",function(data)
    local src = source
    local Player = ESX.GetPlayerFromId(src)
    exports.oxmysql:insert('INSERT INTO hsn_phone_twitter_accounts (username, password, owner, photo) VALUES (@username, @password, @owner, @photo)', {
        ['@username'] = data.username,
...
AddEventHandler("hsn-phone-server-SendNewTweet",function(data)
    if type(data) == "table" then
        ...
        if twtAcc ~= nil then
            ...
            exports.oxmysql:insert('INSERT INTO hsn_phone_tweets (username, message, photo, flastname, date) VALUES (@username, @message, @photo, @flastname, @date)', {
...
AddEventHandler("hsn-phone-server-togglelike",function(tweetId)
    ...
    if TwitterLikes[tweetId] ~= nil then
        ...
        exports.oxmysql:insert('UPDATE hsn_phone_tweets SET togglelikes = @togglelikes , likes = @likes WHERE id = @id', {
...
AddEventHandler("hsn-phone-server-twitter-DeleteTwitterAccount",function()
    local UserName = HSN.GetPlayerTwitterUsername(source)
    if UserName ~= nil then
        exports.oxmysql:execute('DELETE FROM hsn_phone_twitter_accounts WHERE username = @username', {
            ['@username'] = UserName,
        })
        if TwitterAccounts[UserName] ~= nil then
...
AddEventHandler("hsn-phone-server-twitter-logOut",function()
    local UserName = HSN.GetPlayerTwitterUsername(source)
    if UserName ~= nil then
        exports.oxmysql:insert('UPDATE `hsn_phone_twitter_accounts` SET `owner`= "" WHERE username = @username', {
            ['@username'] = UserName
        }, function()
...
AddEventHandler("hsn-phone-server-deletenote",function(noteid)
    ...
    for k,v in pairs(PhoneData[Player.identifier].notes) do
        if v.id == tonumber(noteid) then
            ...
            exports.oxmysql:insert('DELETE FROM hsn_phone_notes WHERE id = @id', {
                ['@id'] = tonumber(noteid)
            })
            break
        end
...
HSN.AddNewNote = function(src,noteData)
    local Player = ESX.GetPlayerFromId(src)
    exports.oxmysql:insert('INSERT INTO hsn_phone_notes (owner, noteData) VALUES (@owner, @noteData)', {
        ['@owner']   = Player.identifier,
...
AddEventHandler("hsn-phone-server-addnewContact",function(data)
    ...
    if data.phonenumber and data.name then
        ...
        if TargetPlayer ~= "" then
            ...
            exports.oxmysql:insert('UPDATE users SET contacts = @contacts WHERE identifier = @identifier', {
...
AddEventHandler("hsn-phone-server-addNewMail",function(data)
    ...
    exports.oxmysql:insert('INSERT INTO hsn_phone_mails (owner, message,date,app) VALUES (@owner, @message, @date, @app)', {
        ['@owner']   = Player.identifier,
        ['@message']   = data.message,
        ["@date"] = data.date,
        ["@app"] = data.app
...
AddEventHandler("hsn-phone-server-deletemail",function(id)
    ...
    for k,v in pairs(PhoneData[Player.identifier].mails) do
        if v.id == tonumber(id) then
            ...
            exports.oxmysql:execute('DELETE FROM hsn_phone_mails WHERE id = @id', {
                ['@id'] = tonumber(id)
            })
...
AddEventHandler("hsn-phone-server-SetCarOut",function(plate)
    if plate ~= nil then
        exports.oxmysql:insert('UPDATE owned_vehicles SET stored = @stored WHERE plate = @plate', {
            ['@stored'] = 0,
            ['@plate'] = plate
        })   
    end
...
AddEventHandler("hsn-phone-server-CreateNewPatientData",function(patientData)
    ...
    if patientData then
        if patientData.name ~= "" and patientData.message ~= "" then
            ...
            exports.oxmysql:insert('INSERT INTO hsn_phone_ambulancepatients (name, message,photo,date, id) VALUES (@name, @message, @photo, @date, @id)', {
    ...
    if patientData and patientData.patient then
        ...
        for k,v in pairs(AmbulancePatientDatas) do
            if v.id == patientData.patient then
                ...
                print("asdasdasd")
                exports.oxmysql:insert('DELETE FROM hsn_phone_ambulancepatients WHERE id = @id', {
    ...
    if bolo then
        if bolo.reason then
            if bolo.player ~= nil or bolo.player ~= "0" or bolo.player ~= 0 then
                ...
                exports.oxmysql:insert('UPDATE users SET bolos = @bolos WHERE identifier = @identifier', {
...
AddEventHandler("hsn-phone-server-SendMessage",function(data)
    ...
    if result[1] then
        exports.oxmysql:insert('UPDATE hsn_phone_messages SET data = @data, date = @date, number = @number WHERE owner = @owner AND number = @number', {
        ...
        exports.oxmysql:insert('INSERT INTO hsn_phone_messages (data, owner, number, date) VALUES (@data, @owner, @number, @date)', {
            ['@owner']   = Player.identifier,
            ['@data']   = json.encode(PhoneData[Player.identifier].messages[data.number]),
            ['@number'] = data.number,
            ['@date'] = os.date('%Y-%m-%d %H:%M:%S')
